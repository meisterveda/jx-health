...
GOPRIVATE := github.com/jenkins-x/jx-apps,github.com/jenkins-x/jx-helpers/v3,github.com/jenkins-x/jx-promote
...
BUILDFLAGS :=  -ldflags \
  " -X $(ROOT_PACKAGE)/pkg/cmd/version.Version=$(VERSION)\
		-X github.com/jenkins-x-plugins/jx-health/pkg/cmd/version.Version=$(VERSION)\
...
darwin-arm64: ## Build for OSX
	CGO_ENABLED=$(CGO_ENABLED) GOOS=darwin GOARCH=arm64 $(GO) $(BUILD_TARGET) $(BUILDFLAGS) -o build/darwin/$(NAME) $(MAIN_SRC_FILE)
	chmod +x build/darwin/$(NAME)
